...
DEFAULT_PACE_SECONDS = 3.2  # documented hourly plans also have a 20 requests/minute burst ceiling
...
def parse_fixture_odds(fixture: dict, odds_payload: dict, *, bookmaker: str = "bet365", observed_at: str | None = None) -> CurrentMarket:
    ...
    return CurrentMarket(
        source=f"5dollarfootballapi:{bookmaker}:latest_pre_match",
        ...
        ah_away_line=-line if line is not None else None,
...
class FiveDollarCurrentProvider:
    ...
    def fetch(self) -> list[CurrentMarket]:
        ...
        if fixtures and not rows:
            raise RuntimeError("Scheduled fixtures were returned but no usable Bet365 multi-market rows parsed")
        return rows
...
def probe_from_env(limit: int = 3) -> dict:
    ...
    if not key:
        return {
            ...
            "rows": 0,
            "note": f"Set GitHub secret {ENV_KEY}. No key is stored in the repository.",
        }
    try:
        account = (_get("/status", key).get("data") or {})
