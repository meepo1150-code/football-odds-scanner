...
def _signal(n,roi,ci):
    if n<30 or roi is None or ci[0] is None:return 'INSUFFICIENT_SAMPLE'
    if roi>0 and ci[0]>50:return 'POSITIVE_CANDIDATE'
    if roi<0 and ci[1]<50:return 'NEGATIVE_CANDIDATE'
    return 'INCONCLUSIVE'
def build(root=Path('.')):
    ...
    def aggregate(keys,core_only=False):
        ...
        for vals,rs in groups.items():
            ...
            out.append({**dict(zip(keys,vals)),'n':len(rs),'settled_n':n,'full_win':pc['WIN'],'half_win':pc['HALF_WIN'],'push':pc['PUSH'],'half_loss':pc['HALF_LOSS'],'full_loss':pc['LOSS'],'favorite_price_win_n':favwin,'underdog_price_win_n':dogwin,'favorite_price_win_pct':round(100*favwin/n,2) if n else None,'underdog_price_win_pct':round(100*dogwin/n,2) if n else None,'push_pct':round(100*pc['PUSH']/n,2) if n else None,'favorite_profit_units':round(profit,4),'favorite_roi_pct':round(roi,2) if roi is not None else None,'favorite_win_wilson95_low':ci[0],'favorite_win_wilson95_high':ci[1],'sample_label':_sample(n),'signal_label':_signal(n,roi,ci)})
