...
def _sample(n):return 'VERY_LOW' if n<30 else 'DEVELOPING' if n<100 else 'MODERATE' if n<300 else 'STRONG_300_PLUS'
def build(root=Path('.')):
    ...
    for fid,ss in sorted(grouped.items()):
        ...
        s=ss[-1];h,a=_score(r);fav=str(s.get('favorite_side') or '').upper();ah=s.get('ah') or {};one=s.get('one_x_two') or {};au=ag.get((fid,str(s.get('observed_at'))),{});settle=None;profit_units=None
        try:
    ...
    def aggregate(keys,core_only=False):
        ...
        for vals,rs in groups.items():
            pc=Counter(x['price_result'] for x in rs);n=sum(pc[x] for x in ('WIN','HALF_WIN','PUSH','HALF_LOSS','LOSS'));favwin=pc['WIN']+pc['HALF_WIN'];dogwin=pc['LOSS']+pc['HALF_LOSS'];ci=_wilson(favwin,n);profits=[_num(x.get('favorite_profit_units')) for x in rs];profits=[x for x in profits if x is not None];profit=sum(profits);roi=(100*profit/len(profits)) if profits else None
            out.append({**dict(zip(keys,vals)),'n':len(rs),'settled_n':n,'full_win':pc['WIN'],'half_win':pc['HALF_WIN'],'push':pc['PUSH'],'half_loss':pc['HALF_LOSS'],'full_loss':pc['LOSS'],'favorite_price_win_n':favwin,'underdog_price_win_n':dogwin,'favorite_price_win_pct':round(100*favwin/n,2) if n else None,'underdog_price_win_pct':round(100*dogwin/n,2) if n else None,'push_pct':round(100*pc['PUSH']/n,2) if n else None,'favorite_profit_units':round(profit,4),'favorite_roi_pct':round(roi,2) if roi is not None else None,'favorite_win_wilson95_low':ci[0],'favorite_win_wilson95_high':ci[1],'sample_label':_sample(n)})
        return sorted(out,key=lambda x:-x['n'])
    def aggregate_ou(keys,core_only=False):
        groups=defaultdict(list)
        for x in outcomes:
            if core_only and not x['ah_core_price']:continue
            groups[tuple(str(x.get(k,'UNKNOWN')) for k in keys)].append(x)
        out=[]
        for vals,rs in groups.items():
            n=len(rs);over=sum(x.get('ou25_result')=='OVER' for x in rs);under=sum(x.get('ou25_result')=='UNDER' for x in rs);priced=[x for x in rs if x.get('ou25_over_price') is not None and x.get('ou25_under_price') is not None]
            op=[x['ou25_over_price']-1 if x.get('ou25_result')=='OVER' else -1 for x in priced];up=[x['ou25_under_price']-1 if x.get('ou25_result')=='UNDER' else -1 for x in priced]
            out.append({**dict(zip(keys,vals)),'n':n,'over_2_5_n':over,'under_2_5_n':under,'over_2_5_pct':round(100*over/n,2) if n else None,'under_2_5_pct':round(100*under/n,2) if n else None,'ou25_priced_n':len(priced),'over_2_5_profit_units':round(sum(op),4) if op else None,'over_2_5_roi_pct':round(100*sum(op)/len(op),2) if op else None,'under_2_5_profit_units':round(sum(up),4) if up else None,'under_2_5_roi_pct':round(100*sum(up)/len(up),2) if up else None,'sample_label':_sample(n)})
        return sorted(out,key=lambda x:-x['n'])
