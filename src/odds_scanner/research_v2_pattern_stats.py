...
def build(root=Path('.')):
    ...
    def aggregate_ou(keys,core_only=False):
        ...
        out=[]
        for vals,rs in groups.items():
            n=len(rs);over=sum(x.get('ou25_result')=='OVER' for x in rs);under=sum(x.get('ou25_result')=='UNDER' for x in rs)
            out.append({**dict(zip(keys,vals)),'n':n,'over_2_5_n':over,'under_2_5_n':under,'over_2_5_pct':round(100*over/n,2) if n else None,'under_2_5_pct':round(100*under/n,2) if n else None,'sample_label':_sample(n)})
        return sorted(out,key=lambda x:-x['n'])
