...
def _ou25(s,h,a):
    ou=s.get('over_under_2_5') or s.get('ou_2_5') or {}
    over=_num(ou.get('over') if isinstance(ou,dict) else None);under=_num(ou.get('under') if isinstance(ou,dict) else None)
    total=h+a
...
def build(root=Path('.')):
    ...
    for fid,ss in sorted(grouped.items()):
        ...
        s=ss[-1];h,a=_score(r);fav=str(s.get('favorite_side') or '').upper();ah=s.get('ah') or {};one=s.get('one_x_two') or {};au=ag.get((fid,str(s.get('observed_at'))),{});settle=None
