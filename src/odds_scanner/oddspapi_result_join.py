...
def normalize_result(fixture:dict)->dict|None:
    ...
    if fixture.get("statusId")!=2 or fixture.get("fixtureId") is None:return None
    hg=ag=None; source=None
    ...
    if isinstance(result,dict) and isinstance(result.get("participant1Score"),(int,float)) and isinstance(result.get("participant2Score"),(int,float)):
        hg,ag=int(result["participant1Score"]),int(result["participant2Score"]); source="ODDSPAPI_FIXTURE_SCORES_RESULT"
    else:
        # Backward-compatible explicit flat score forms used by fixtures/tests; no period aggregation.
        for a,b in (("participant1Score","participant2Score"),("homeScore","awayScore"),("score1","score2")):
            if isinstance(fixture.get(a),(int,float)) and isinstance(fixture.get(b),(int,float)):
                hg,ag=int(fixture[a]),int(fixture[b]);source="ODDSPAPI_FINISHED_FIXTURE_EXPLICIT_SCORE";break
...
def join_rows(ticks:list[dict],results:list[dict])->tuple[list[dict],dict]:
    index={str(r.get("fixture_id")):r for r in results if r.get("fixture_id") is not None};joined=[];missing=[]
    for row in ticks:
        fid=str(row.get("fixture_id"));result=index.get(fid)
        if not result:missing.append(fid);continue
        item=dict(row);item.update({"ft_home_goals":result["ft_home_goals"],"ft_away_goals":result["ft_away_goals"],"result_source":result.get("result_source"),"result_join_status":"JOINED","promotion_eligible":False});joined.append(item)
