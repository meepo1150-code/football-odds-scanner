...
def run(root=Path('.')):
    ...
    for r in _rows(root/SNAP): groups[str(r.get('fixture_id'))].append(r)
    features=[]
    for fid,rs in groups.items():
        ...
        # Preserve whatever canonical snapshot keys exist; calculate deltas only when numeric.
        for key in ('ah_line','favorite_price','ou_line','over_price','under_price','home_price','draw_price','away_price'):
            av,bv=_num(a.get(key)),_num(b.get(key)); row['first_'+key]=av; row['last_'+key]=bv; row['delta_'+key]=None if av is None or bv is None else round(bv-av,4)
        features.append(row)
    ...
    report={'schema_version':'1.0','status':'MOVEMENT_BUILT','fixtures_with_two_plus_snapshots':len(features),'source_snapshot_rows':sum(len(v) for v in groups.values())}
