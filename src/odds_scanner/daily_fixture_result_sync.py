...
def run(root: Path = Path('.')) -> dict:
    ...
    report = {'schema_version':'1.3','classification':'RESEARCH_V2_DAILY_FIXTURE_RESULT_SYNC','generated_at':now.isoformat(),'football_day':football_day,'coverage_days':coverage_days,'window_start':start_utc.isoformat(),'window_end':end_utc.isoformat(),'research_only':True,'odds_requested':False}
    ...
    complete = str(previous.get('football_day')) == football_day and previous.get('status') in {'FIXTURES_SYNCED','ALREADY_SYNCED'} and all(x in prior_coverage for x in coverage_days)
    if complete:
        report.update(status='ALREADY_SYNCED', fixture_requests_used=0, football_day_fixtures=len(existing), previous_day_fixtures=len(existing_prev), persisted_fixture_rows=sum(1 for line in fixtures_path.read_text(encoding='utf-8').splitlines() if line.strip()) if fixtures_path.exists() else 0, completion_marker=True)
    else:
        ...
        if not key: report.update(status='API_KEY_NOT_CONFIGURED', fixture_requests_used=0, completion_marker=False)
            ...
            try:
                # One /fixtures request covers both yesterday and today. This preserves exact
                # OddsPAPI fixture -> Flashscore IDs needed for deterministic result backfill.
                ...
                report.update(status='FIXTURES_SYNCED', fixture_requests_used=1, api_fixtures_returned=len(api_rows), football_day_fixtures=counts[football_day], previous_day_fixtures=counts[previous_day], persisted_fixture_rows=total, completion_marker=True)
